	...
	if LocalPlayer():GetNWBool("ShouldShowHUD", false) then
		...
		if GetGlobalBool( "InRound", false ) or GetGlobalBool( "InPreRound", false) then
			...
			elseif GetGlobalBool( "InPreRound", false) then
				draw.SimpleTextOutlined( "Pre-Round Time: "..util.ToMinutesSeconds( GetGlobalFloat( "RoundStartTime", 0.0 )-CurTime() ), "DermaLarge", ScrW()/2, 40, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black )
			end
		end
